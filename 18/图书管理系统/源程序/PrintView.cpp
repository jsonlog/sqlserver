...
void CPrintView::OnInitialUpdate()
{
	...
	pDC=this->GetDC();
	//逻辑设备大小
	caps.cx=pDC->GetDeviceCaps(LOGPIXELSX);
	caps.cy=pDC->GetDeviceCaps(LOGPIXELSY);/**/
}
...
void CPrintView::DrawReportTitle(CDC* pDC,LPCSTR r_title,bool printed)
{
	...
	if (printed)
	{
		...
		pDC->DrawText(line,CRect(0,factrate.y*20, size.cx,factrate.y*100),DT_CENTER|DT_TOP|DT_SINGLELINE);
		pDC->SetTextColor(RGB(0,0,0));
		pDC->DrawText(r_title,CRect(0,factrate.y*10, size.cx,factrate.y*100),DT_CENTER|DT_TOP|DT_SINGLELINE);	
	}
	else
	{
		pDC->SetTextColor(RGB(255,0,0));
...
void CPrintView::DrawBody(CDC *pDC,bool printed)
{
	...
	int fieldinterval = 0;
	if (printed)
		fieldinterval = (size.cx- factrate.x*report.lmargin- factrate.x*(report.rmargin+fieldwidth))/(report.fieldcount-1);
	else
...
//	pagesize.cy= mode->dmPaperWidth;
	//获取打印机每英寸像素数
	pagesize.cx =pDC->GetDeviceCaps(LOGPIXELSX);
	pagesize.cy =pDC->GetDeviceCaps(LOGPIXELSY);	
