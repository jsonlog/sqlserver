...
void CDlgBookPrice2::OnQuery() 
{
	...
	CString sql;
	sql.Format("select barcode,bookname,shortname,author,bookconcern,price from tb_bookinfo where barcode in (select barcode from tb_fixprice) and %s = '%s'",field,c_value);
	m_pRs = m_pCon->Execute((_bstr_t)sql,NULL,adCmdText);
	int row = 0;
	while (! m_pRs->adoEOF)
	{
