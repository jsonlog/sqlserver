...
void CDlgOperator1::AddOperator()
{
	...
	if (!InfoIsNull())
	{
		...
		CString sql;
		sql.Format("insert into tb_operator values ('%s','%s')",c_operator,c_password);
...
bool CDlgOperator1::InfoIsNull()
{
	...
	m_operator.GetWindowText(c_operator);
	m_password.GetWindowText(c_password);
	if (c_operator.IsEmpty()||c_password.IsEmpty())
		return true;
	return false;
}
	...
	if (!InfoIsNull())
	{
		if (m_list.GetSelectionMark()!=-1)
		{
			if (MessageBox("确实要修改数据吗?","提示",MB_YESNO|MB_ICONINFORMATION)==IDYES)
			{
				...
				CString sql;
				sql.Format("update tb_operator set name = '%s',password = '%s' where name = '%s'",c_operator,c_password,c_name);
				m_pRs->raw_Close();
				try
				{
					m_pRs->Open((_variant_t)sql,m_pCon.GetInterfacePtr(),adOpenKeyset,adLockOptimistic,adCmdText);
