...
BOOL CDlgBookInput2::OnInitDialog() 
{
	...
	m_auxilist.SetExtendedStyle(LVS_EX_FULLROWSELECT
...
void CDlgBookInput2::EditChange()
{
	...
	m_list.edit.GetWindowText(str);
		...
		{
			m_pRs->raw_Close();
			m_pRs->Open((_bstr_t)sql,m_pCon.GetInterfacePtr(),adOpenKeyset,
			...
			if (m_pRs->RecordCount>0)
			{
				...
				while(! m_pRs->adoEOF)
				{
					...
					GetFields()->GetItem((long)m)->Value);
					m_pRs->MoveNext();
					i +=1;
				}
				ShowListInfo();
